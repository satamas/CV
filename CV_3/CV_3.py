...
mask[CUTOFF_FREQUENCY:fft.shape[0] - CUTOFF_FREQUENCY, 0:fft.shape[1]] = 1
mask[0:fft.shape[0], CUTOFF_FREQUENCY:fft.shape[1] - CUTOFF_FREQUENCY] = 1

filtered_fft = fft * mask

processedImage = np.fft.ifft2(filtered_fft)
cv2.imwrite('filtered.bmp', np.abs(processedImage))
